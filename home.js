...
function displayProducts(data) {
  let myProductsContainer = document.querySelector("#myProducts");
  ...
  let myproductContent = data.map(function (product) {
    ...
    <div class="group flex flex-col justify-between overflow-hidden bg-white shadow-sm transition hover:shadow-md border border-gray-100 rounded-sm">
        <div class="h-64 w-full overflow-hidden bg-gray-50 flex items-center justify-center">
            <img src="${product.thumbnail}" alt="${product.title}" class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
        </div>
        <div class="flex flex-col gap-2 p-3 flex-grow">
            <div class="text-base font-bold text-neutral-900">$${product.price}</div>
            <h3 class="truncate text-xs text-gray-700 font-semibold" title="${product.title}">${product.title}</h3>
            <p class="line-clamp-2 text-[11px] text-gray-500 leading-normal">${product.description}</p>
            <span class="text-[10px] font-medium text-amber-600 bg-amber-50 self-start px-1 mt-1 rounded-sm mb-1">#${product.category.replace("-", "")}</span>
            <button onclick="addToCart(${product.id})" class="mt-auto px-4 self-start rounded bg-black py-2 text-xs font-semibold uppercase tracking-wider text-white transition hover:bg-neutral-800 active:scale-[0.98]">
                Add to Cart
            </button>
        </div>
    </div>
  ...
  myProductsContainer.innerHTML = myproductContent.join("");
}
...
function filterProducts() {
  ...
  if (productFilter.length === 0) {
    document.querySelector("#myProducts").innerHTML = `
        <p class="text-gray-500 font-medium p-8 text-center col-span-full">No products found matching your search.</p>
    `;
  } else {
